...
function PostComment() {
    ...
    useEffect(() => {
      fetch('https://flickfeeds-602d4f3e68d7.herokuapp.com/posts')
        .then(response => response.json())
        .then(data => {
          const lastPostIndex = data.posts.length - 1;
          const lastCommentIndex = data.posts[lastPostIndex].Comments.length - 1;
          const lastCommentText = data.posts[lastPostIndex].Comments[lastCommentIndex].CommentText;
          setLastComment(lastCommentText);
        })
                    ...
                    <div class="col-md-11 col-lg-9 col-xl-6">
                        <div class="card text-light" style={{ boxShadow: "5px 5px 10px grey", backgroundColor: "black" }}>
                            <div class="card-body">
                                ...
                                <div>
                                <p className="mb-1 pb-1 ms-5">{lastComment}</p>
                                </div>
                            </div>
                        </div>
                    </div>
